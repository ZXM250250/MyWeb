...
type Client struct {
	cc      codec.Codec //是消息的编解码器，和服务端类似，用来序列化将要发送出去的请求，以及反序列化接收到的响应。
	opt     *Option
	sending sync.Mutex       // protect following  //是一个互斥锁，和服务端类似，为了保证请求的有序发送，即防止出现多个请求报文混淆。
	...
	// 任意一个值置为 true，
	// 则表示 Client 处于不可用的状态，但有些许的差别，closing 是用户主动关闭的，
	//即调用 Close 方法，而 shutdown 置为 true 一般是有错误发生。
	shutdown bool // server has told us to stop
	closing  bool // user has called Close closing 和 shutdown
}
...
func (client *Client) receive() {
	var err error
	for err == nil {
		var h codec.Header //读取头部的时候 就已经被堵塞了
...
//即发送 Option 信息给服务端。协商好消息的编解码方式之后，再创建一个子协程调用 receive() 接收响应。
